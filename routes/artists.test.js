...
describe("GET /artists", function () {
  ...
  test("works: filtering", async function () {
    const resp = await request(app).get(`/artists`).query({ hasEquity: true });
    expect(resp.body).toEqual({
      artists: [
        {
          id: expect.any(Number),
...
describe("GET /artists/:id", function () {
  test("works for anon", async function () {
    const resp = await request(app).get(`/artists/${testArtistIds[0]}`);
    expect(resp.body).toEqual({
      artist: {
        id: testArtistIds[0],
        title: "J1",
  ...
  test("not found for no such artist", async function () {
    const resp = await request(app).get(`/artists/0`);
